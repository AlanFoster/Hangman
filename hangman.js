var Plugin = (function () {
    ...
    var Plugin = function () {
        ...
        this.triggers = [
            ...
            [/^!guess (.)/, function (channel, user, mask, match) {
                this.tryGuess(channel, user, mask, match[1]);
            }]
        ];
...
(function () {
    ...
    var TestSuite = (function tests(createGUI) {
        var log = createGUI && document ? (function () {
            document.write("<pre>");
            var f = function (message) {
                document.write(escape(message).replace(/%(..)/g, "&#x$1;").replace("\n", "<br />") + "<br />");
            };
            return f;
        })() : function (m) {
                console.log(m);
            ...
            assertDescriptions.forEach(function (assertDescription) {
                (function (assertName, assertFunction, logMessage) {
                    assertObject[assertName] = function () {
                        ...
                        if (!assertPassed) {
                            log("FAILED  :: " + assertName + " : " + formattedLogMessage);
                            throw new Error("FAILED  :: " + assertName + " : " + formattedLogMessage);
                        } else {
                            log("PASSED :: " + assertName + " : " + formattedLogMessage);
                        }
                    };
        ...
        var TestRunner = (function (tests, continueOnError) {
            ...
            for (var i in tests) {
                ...
                try {
                    ...
                    log(["Test Failed for " + i, "Error Message : ", e.message, "Stack Trace : ", e.stack].join("\n"));
            ...
            log("\n");
            log(["TESTS COMPLETE", "Total tests ran : " + testStats.testCount, "Total test count : " + testStats.totalTestCount, "passed tests : " + testStats.passedTests, "Failed Tests : " + testStats.failedTests].join("\n"));
        });
    ...
    (function (Assert, TestRunner) {
        ...
        var tests = {
            setUp: function () {
                instance = new Plugin();
                instance.say = function (channel, message) {
                    console.log(message);
                }
            },
            tearDown: function () {
            ...
            testValidLetter: function () {
                instance.getRandomWord = function () {
                    return "hello";
                };
            ...
            testMaxGuessesEndsGame: function () {
                ...
                for (var i in guesses) {
                    Assert.assertTrue(game.hasGuessesLeft());
